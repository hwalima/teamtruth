                  ...
                  {usage_history?.data && usage_history.data.length > 0 ? (
                    usage_history.data.map((item: any, index: number) => (
                      <TableRow key={index}>
                        ...
                        <TableCell><div className="flex items-center gap-1.5 text-sm whitespace-nowrap text-gray-500"><Calendar className="h-3.5 w-3.5 shrink-0" />{window.appSettings?.formatDateTime(item.used_at) || item.used_at}</div></TableCell>
                      </TableRow>
                    ))
                  ) : (
                    <TableRow>
