...
import { CheckCircle, XCircle, Clock, MessageSquare, LayoutGrid, List, Calendar, User, Timer, DollarSign, Eye, MoreHorizontal, Check, X, ChevronUp, ChevronDown, ChevronsUpDown } from 'lucide-react';
...
export default function TimesheetApprovals({ approvals, filters, userWorkspaceRole, permissions }: Props) {
    ...
    const clearFilters = () => {
        ...
        setSearchTerm('');
        navigate({ page: 1, view: activeView, per_page: filters.per_page || '10' });
    };
    ...
    return (
        <PageTemplate 
            title={t('Approvals')}
            description={t('Review and manage timesheet approvals.')} 
                ...
                <SearchAndFilterBar
                    ...
                    filters={[
                        ...
                        },
                        {
                            ...
                            label: t('From Date'),
                            type: 'date',
                            value: filters.from_date ? new Date(filters.from_date) : undefined,
                            onChange: (date: Date | undefined) => navigate(buildParams({ page: 1, from_date: date ? date.toISOString().split('T')[0] : undefined })),
                        },
                        {
                            name: 'to_date',
                            label: t('To Date'),
                            ...
                            onChange: (date: Date | undefined) => navigate(buildParams({ page: 1, to_date: date ? date.toISOString().split('T')[0] : undefined })),
                        },
                    ]}
                ...
                {(activeView === 'grid' || !activeView) ? (
                            ...
                            <Card key={approval.id} className="hover:shadow-md transition-shadow flex flex-col h-full">
                                <CardHeader className="pb-3">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className={`inline-flex shrink-0 items-center rounded-md px-2 py-1 text-xs font-medium ${getStatusColor(approval.status)}`}>
                                                    ...
                                                    <TooltipTrigger asChild>
                                                        <div>
                                                            <ApprovalDialog
                                                                ...
                                                                action="approve"
                                                                trigger={
                                                                    <Button size="icon" variant="ghost" className="h-7 w-7 text-gray-500 hover:text-gray-700 hover:bg-gray-50">
                    ...
                    <div className="bg-white rounded-lg shadow overflow-hidden">
                                            ...
                                            <TableCell className="py-2.5">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-xs text-gray-500">{approval.timesheet.user.email || ''}</div>
                                                    </div>
                                                </div>
                                            </TableCell>
                                            <TableCell className="py-2.5">
                                                <div className="flex items-start gap-1.5 mb-2 text-sm whitespace-nowrap text-gray-500">
                                                    <Calendar className="h-3.5 w-3.5 shrink-0 mt-1" />
                                                    <div>
                                                        <div className="font-medium">
                                                            {window.appSettings.formatDateTime(new Date(approval.timesheet.start_date), false)}
                                                        </div>
                                                        <div className="font-medium">
                                                            to {window.appSettings.formatDateTime(new Date(approval.timesheet.end_date), false)}
                                                        </div>
                                                    </div>
                                                </div>
                                            </TableCell>
                                                                ...
                                                                <TooltipTrigger asChild>
                                                                    <div>
                                                                        <ApprovalDialog
                                                                            ...
                                                                            action="approve"
                                                                            trigger={
                                                                                <Button size="icon" variant="ghost" className="h-8 w-8 text-gray-500 hover:text-gray-700 hover:bg-gray-50">
                                                                                    <Check className="h-4 w-4" />
                                                                                </Button>
                                                                            }
                                                                        />
                            ...
                            {approvals?.links && approvals.data.length > 0 && (
                                    <Pagination
                                        from={approvals?.from || 0}
                                        to={approvals?.to || 0}
                                        total={approvals?.total || 0}
                                        links={approvals?.links}
                                        entityName={t('approvals')}
                                        currentPerPage={filters.per_page?.toString() || '10'}
                                        perPageOptions={[10, 25, 50, 100]}
                                        onPerPageChange={(value) => navigate(buildParams({ page: 1, per_page: value }))}
                                        onPageChange={(url) => {
