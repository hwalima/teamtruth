...
export default function PayoutRequests({ userType, payoutRequests, settings, stats, currencySymbol }: PayoutRequestsProps) {
  ...
  key: 'amount',
  label: t('Amount'),
  render: (value: number) => <span className="font-mono">{currencySymbol}{value}</span>
},
      {
        key: 'status',
        label: t('Status'),
  ...
  {stats.availableBalance < settings.threshold_amount
    ? <>{t('You need at least')} <span className="font-mono">{currencySymbol}{settings.threshold_amount}</span> {t('to request a payout')}</>
    : <>{t('You can request up to')} <span className="font-mono">{currencySymbol}{stats.availableBalance}</span> {t('for payout')}</>}
</p>
