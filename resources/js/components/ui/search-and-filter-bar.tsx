...
export function SearchAndFilterBar({
    ...
    const activeFilters = filters.filter(f => {
        ...
        if (typeof f.value === 'string' && (f.value === '_empty_' || f.value === 'all' || f.value === '')) return false;
        return true;
    });
    ...
    const getFilterLabel = (filter: FilterOption) => {
        ...
        if (filter.value instanceof Date) {
            return window?.appSettings?.formatDateTime(filter.value as Date, false) || (filter.value as Date).toLocaleDateString();
        }
    ...
    const removeFilter = (filter: FilterOption) => {
        if (filter.type === 'select') {
            // Use first option value as the empty/reset value
            const emptyValue = filter.options?.[0]?.value ?? '_empty_';
            filter.onChange(emptyValue);
        } else {
