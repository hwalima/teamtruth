                ...
                {allWorkspaces.map((workspace) => (
                    ...
                    >
                        <div 
                            ...
                            onClick={() =>{
                                // In demo mode, save to cookie immediately before router call (like language switcher)
                                if (isDemoMode()) {
                                    const cookieKey = isSaasMode
                                        ? 'saas_current_workspace_id'
                                        : 'nonsaas_current_workspace_id';
                                    setCookie(cookieKey, String(workspace.id));
                                }
                        ...
                        >
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                {!workspace.is_owner && (
                                    <div className="text-xs text-muted-foreground">Shared • <span className="bg-primary/10 text-primary px-1.5 py-0.5 rounded-full text-xs capitalize font-medium">
                                        {workspace.pivot?.role || 'No Role'}</span></div>
                                )}
                            </div>
                        </div>
