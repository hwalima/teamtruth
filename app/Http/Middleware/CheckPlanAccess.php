...
class CheckPlanAccess
{
    public function handle(Request $request, Closure $next)
    {
        ...
        if (!$user->current_workspace_id) {
            auth()->logout();
            return redirect()->route('login')->with('status', __('No workspace assigned. Please contact support.'))->with('statusType', 'error');
        }
        ...
        if (!$owner) {
            auth()->logout();
            return redirect()->route('login')->with('status', __('Workspace owner not found. Please contact support.'))->with('statusType', 'error');
        }
