...
class NoteController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $transformNote = function ($note) {
            ...
            if ($note->assign_to) {
                $assignedUsers = $note->assigned_users;
                $assignedUsers->transform(function ($u) {
                    $u->avatar = check_file($u->avatar)
                        ? get_file($u->avatar)
                        : get_file('avatars/avatar.png');
                    return $u;
                });
                $note->setRelation('assigned_users', $assignedUsers);
            }
            $note->setAppends([]);
            return $note;
        };

        $isKanban = $request->get('view_mode') === 'kanban';

        // Get paginated results
        if ($isKanban) {
            $personal_notes = ['data' => $personalQuery->get()->map($transformNote)];
            $shared_notes = ['data' => $sharedQuery->get()->map($transformNote)];
        } else {
            $personal_notes = $personalQuery->paginate((int)($perPage / 2), ['*'], 'notes_page')->through($transformNote);
            $shared_notes = $sharedQuery->paginate((int)($perPage / 2), ['*'], 'notes_page')->through($transformNote);
        }
        ...
        $totalPersonalNotes = Note::where('type', 'personal')
            ->where('workspace', $currentWorkspace->id)
            ->where('created_by', Auth::user()->id)
            ->count();

        $totalSharedNotes = Note::where('type', 'shared')
            ->where('workspace', $currentWorkspace->id)
            ->whereRaw("find_in_set('" . Auth::user()->id . "', notes.assign_to)")
            ->count();
        ...
        return Inertia::render('notes/Index', [
            ...
            'total_personal_notes' => $totalPersonalNotes,
            'total_shared_notes' => $totalSharedNotes,
            'filters' => $request->only(['per_page', 'search', 'type', 'sort_field', 'sort_direction', 'view_mode', 'notes_page']),
            'permissions' => [
