...
class ProjectBudgetController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if (in_array($userWorkspaceRole, ['manager', 'client'])) {
            $projectsQuery->where(function($q) use ($user) {
                $q->whereHas('members', function($q) use ($user) { $q->where('user_id', $user->id); })
                  ->orWhereHas('clients', function($q) use ($user) { $q->where('user_id', $user->id); })
                  ->orWhere('created_by', $user->id);
            });
        }
        ...
        return Inertia::render('budgets/Form', [
            'mode' => 'create',
            'projects' => $projectsQuery->get(['id', 'title']),
            'allProjects' => $projectsQuery->get(['id', 'title']),
            'currencies' => $currencies,
            'workspace' => $workspace,
        ]);
    }
    ...
    public function edit(ProjectBudget $budget)
    {
        ...
        $budget->load(['project', 'categories']);

        $user = auth()->user();
        $workspace = $user->currentWorkspace;
        $userWorkspaceRole = $workspace->getMemberRole($user);

        $allProjectsQuery = Project::forWorkspace($workspace->id)->orderBy('title');
        ...
        return Inertia::render('budgets/Form', [
            ...
            'budget' => $budget,
            'allProjects' => $allProjectsQuery->get(['id', 'title']),
            'projects' => $allProjectsQuery->get(['id', 'title']),
            'currencies' => $currencies,
            'workspace' => $workspace,
        ]);
    }
    ...
    public function update(Request $request, ProjectBudget $budget)
    {
        ...
        return redirect()->route('budgets.index')->with('success', __('Budget updated successfully!'));
    }
