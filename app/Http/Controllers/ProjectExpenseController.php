...
class ProjectExpenseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $projectIds = $projectsCollection->pluck('id');

        $allProjectExpenses = ProjectExpense::with(['submitter', 'budgetCategory', 'task'])
        ...
        // Load all project expenses as flat array (mirrors Budget Show's budget.expenses approach)
        $projectExpensesQuery = ProjectExpense::with(['project', 'submitter', 'budgetCategory', 'task'])
            ->whereIn('project_id', $projectIds);

        // Apply same filters as main expenses query
        if ($request->category_id) {
            $projectExpensesQuery->where('budget_category_id', $request->category_id);
        }
        if ($request->status) {
            $projectExpensesQuery->where('status', $request->status);
        }
        if ($request->search) {
            $projectExpensesQuery->where(function ($q) use ($request) {
                $q->where('title', 'like', '%' . $request->search . '%')
                    ->orWhere('description', 'like', '%' . $request->search . '%')
                    ->orWhere('vendor', 'like', '%' . $request->search . '%')
                    ->orWhereHas('submitter', function ($subQ) use ($request) {
        ...
        $projectExpenses = $projectExpensesQuery->get()
            ->each(function ($expense) {
                if ($expense->submitter) {
                    $expense->submitter->avatar = check_file($expense->submitter->avatar)
                        ? get_file($expense->submitter->avatar)
        ...
        // Append computed fields to categories
        $projectsCollection->each(function ($project) use ($projectExpenses) {
            $budget = $project->getRelation('budget');
            if ($budget && is_object($budget)) {
                $budget->categories->each(function ($category) use ($projectExpenses) {
                    $category->append(['total_spent', 'utilization_percentage']);
                    $catExp = $projectExpenses->where('budget_category_id', $category->id);
                    $category->expense_count = $catExp->count();
                    $category->pending_count = $catExp->where('status', 'pending')->count();
                });
                $allExp = $projectExpenses->where('project_id', $project->id);
                $project->expense_count = $allExp->count();
                $project->total_expense_amount = $allExp->sum('amount');
            }
        });
        ...
        return Inertia::render('expenses/Index', [
            'expenses' => $expenses,
            'projects' => $projects,
            'project_expenses' => $projectExpenses->values(),
            'categories' => $categories,
            'members' => $members,
            'filters' => $request->only(['project_id', 'category_id', 'status', 'submitted_by', 'search', 'per_page', 'sort_by', 'sort_order', 'selected_project', 'selected_category', 'visible_projects']),
            'project_name' => $request->project_name,
            'userWorkspaceRole' => $userWorkspaceRole,
            'all_project_expenses' => $allProjectExpenses->values(),
            'permissions' => [
                ...
                'expense_approve' => $this->checkPermission('expense_approve'),
                'expense_reject' => $this->checkPermission('expense_reject'),
            ]
        ]);
    }
    ...
    public function update(Request $request, ProjectExpense $expense)
    {
        ...
        $redirectUrl = $request->input('redirect_url', route('expenses.index'));

        return redirect()->to($redirectUrl)->with('success', __('Expense updated successfully!'));
    }
