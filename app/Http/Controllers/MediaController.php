...
class MediaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Folder filter: pass folder_id=0 for root, or an actual folder ID
        if ($request->has('folder_id')) {
            $folderId = $request->input('folder_id');
            $query->where('folder_id', $folderId === '0' || $folderId === null ? null : (int) $folderId);
        }
    ...
    public function download($id)
    {
        ...
        // Check lock access on the file itself or its containing folder
        if ($mediaItem) {
            if ($mediaItem->is_locked && !$this->canAccessLocked($user, 'file', $mediaItem->id)) {
                abort(403, __('Access denied'));
            }
            if ($mediaItem->folder_id) {
                $folder = MediaFolder::find($mediaItem->folder_id);
                if ($folder && $folder->is_locked && !$this->canAccessLocked($user, 'folder', $folder->id)) {
                    abort(403, __('Access denied - folder is locked'));
                }
            }
        }
    ...
    // ── Lock / Access endpoints ──────────────────────────────────────────────

    public function toggleLock(int $id)
    {
        $user = auth()->user();
        $mediaItem = $this->resolveMediaItem($id, $user);
        $mediaItem->is_locked = !$mediaItem->is_locked;
        $mediaItem->save();

        return response()->json([
            'message'   => $mediaItem->is_locked ? __('File locked') : __('File unlocked'),
            'is_locked' => $mediaItem->is_locked,
        ]);
    }

    public function accesses(int $id)
    {
        $user = auth()->user();
        $mediaItem = $this->resolveMediaItem($id, $user);

        $list = MediaAccess::where('resource_type', 'file')
            ->where('resource_id', $mediaItem->id)
    ...
    public function storeAccess(Request $request, int $id)
    {
    ...
    public function destroyAccess(Request $request, int $id)
    {
    ...
    public function moveTo(Request $request, int $id)
    {
        $request->validate(['folder_id' => 'nullable|integer|exists:media_folders,id']);

        $user = auth()->user();
        $mediaItem = $this->resolveMediaItem($id, $user);
        $mediaItem->folder_id = $request->input('folder_id');
        $mediaItem->save();

        return response()->json(['message' => __('File moved'), 'folder_id' => $mediaItem->folder_id]);
    }

    // ── Helpers ─────────────────────────────────────────────────────────────

    private function resolveMediaItem(int $mediaItemId, $user): MediaItem
    {
        $query = MediaItem::where('id', $mediaItemId);
    ...
    private function canAccessLocked($user, string $type, int $resourceId): bool
    {
        if ($user->type === 'superadmin') {
            return true;
        }
        ...
        if ($type === 'file') {
            $item = MediaItem::find($resourceId);
            if ($item && $item->user_id === $user->id) {
                return true;
            }
        }

        return MediaAccess::where('resource_type', $type)
