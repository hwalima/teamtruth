...
class SystemSettingsController extends Controller
{
    ...
    public function getIctTicketSettings()
    {
        $user = auth()->user();
        $userId = $user->id;
        $workspaceId = $user->type === 'company' ? $user->current_workspace_id : null;

        $defaults = \App\Models\IctTicket::SUBSIDIARIES;

        return response()->json([
            'subsidiaries'        => json_decode(getSetting('ict_subsidiaries', json_encode(\App\Models\IctTicket::SUBSIDIARIES), $userId, $workspaceId), true) ?? \App\Models\IctTicket::SUBSIDIARIES,
            'departments'         => json_decode(getSetting('ict_departments',  json_encode(\App\Models\IctTicket::DEPARTMENTS),  $userId, $workspaceId), true) ?? \App\Models\IctTicket::DEPARTMENTS,
            'categories'          => json_decode(getSetting('ict_categories',   json_encode(\App\Models\IctTicket::CATEGORIES),   $userId, $workspaceId), true) ?? \App\Models\IctTicket::CATEGORIES,
            'sla_response_hours'  => (int) getSetting('ict_sla_response_hours',  4, $userId, $workspaceId),
            'sla_resolution_hours'=> (int) getSetting('ict_sla_resolution_hours', 24, $userId, $workspaceId),
        ]);
    }

    public function updateIctTicketSettings(Request $request)
    {
        $request->validate([
            'subsidiaries'         => 'nullable|array',
            'subsidiaries.*'       => 'string|max:100',
        ...
        updateSetting('ict_subsidiaries',        json_encode($request->input('subsidiaries', [])),   $userId, $workspaceId);
        updateSetting('ict_departments',         json_encode($request->input('departments', [])),    $userId, $workspaceId);
        updateSetting('ict_categories',          json_encode($request->input('categories', [])),     $userId, $workspaceId);
        updateSetting('ict_sla_response_hours',  $request->input('sla_response_hours', 4),           $userId, $workspaceId);
        updateSetting('ict_sla_resolution_hours',$request->input('sla_resolution_hours', 24),        $userId, $workspaceId);

        return redirect()->back()->with('success', __('ICT ticket settings saved.'));
    }
